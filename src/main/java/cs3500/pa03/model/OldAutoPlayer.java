 ...
 */
public class OldAutoPlayer implements Player {
  private String name = "yashjay1";
  private GameBoard board;
  private int height;
  private int width;
  private List<Coord> hitShots;
  private int turns;
  private ArrayList<Coord> takenShots = new ArrayList<>();
  ...
  public List<Coord> takeShots() {
    ...
    while (shots.size() < this.turns) {
      int x = random.nextInt(height);
      int y = random.nextInt(width);

      Coord shot = new Coord(x, y);

      if (!shots.contains(shot) && x >= 0 && x < height && y >= 0 && y < width &&
          !takenShots.contains(shot)) {
        shots.add(shot);
        takenShots.add(shot);
      }
    }
  ...
  public List<Coord> reportDamage(List<Coord> opponentShotsOnBoard) {
    this.hitShots = board.hit(opponentShotsOnBoard);
    board.displayBoard(true);
    this.turns = board.numOfTurns();
    return hitShots;
  }
